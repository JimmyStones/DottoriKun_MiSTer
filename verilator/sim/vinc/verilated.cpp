...
void VL_WRITEF(const char* formatp, ...) VL_MT_SAFE {
	...
	// Get rid of annoying extra LF before printing to console
	std::string log = console->prefix + output;
	
	int len = log.length();
	size_t find = log.find_last_of("\n");
	if (find < len) { log = log.substr(0, len - 1); }
